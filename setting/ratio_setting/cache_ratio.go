...
func GetCacheRatio(name string) (float64, bool) {
	if ratio, ok := GetMetadataCacheRatio(name); ok {
		return ratio, true
	}
	if ratio, ok := GetOfficialCacheRatio(name); ok {
		return ratio, true
	}
...
func GetCreateCacheRatio(name string) (float64, bool) {
	if ratio, ok := GetMetadataCreateCacheRatio(name); ok {
		return ratio, true
	}
	if ratio, ok := GetOfficialCreateCacheRatio(name); ok {
		return ratio, true
	}
