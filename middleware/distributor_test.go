...
func TestRouteAutoGroupForRequestPathKeepsAutoForRouteRules(t *testing.T) {
...
func TestRouteAutoGroupForRequestPathDoesNotOverrideResponsesRules(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	...
	assert.Equal(t, "auto", common.GetContextKeyString(c, constant.ContextKeyUsingGroup))
	assert.Equal(t, "auto", common.GetContextKeyString(c, constant.ContextKeyTokenGroup))
	assert.Empty(t, common.GetContextKeyStringSlice(c, constant.ContextKeyRouteAutoGroups))
}
